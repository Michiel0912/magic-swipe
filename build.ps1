...
function Find-BuildTools([string]$sdkRoot) {
    ...
    $need = @('aapt2.exe','d8.bat','zipalign.exe')
    foreach ($n in $need) { if (-not (Test-Path (Join-Path $dir $n))) { return $null } }
    return $dir
}
function Find-ReproApkSigner([string]$sdkRoot) {
    # F-Droid/apksigcopier documents a compatibility issue with signatures
    # generated by apksigner >= 35 unless special alignment handling is used.
    # Use the known-compatible Build-Tools 34 signer while keeping 36.0.0 for
    # aapt2/D8/zipalign. This does not change the signing certificate/key.
    $p = Join-Path (Join-Path (Join-Path $sdkRoot 'build-tools') '34.0.0') 'apksigner.bat'
...
function Test-UsableSdk([string]$sdkRoot) {
    ...
    return [bool](Find-BuildTools $sdkRoot) -and [bool](Find-ReproApkSigner $sdkRoot)
}
...
if (-not $SdkRoot) {
    foreach ($c in $candidates) {
        ...
        if ($sm) {
            ...
            Write-Host "Installing reproducible Android build/signing components via: $sm"
            $yes = (1..40 | ForEach-Object { 'y' })
            $yes | & $sm --sdk_root=$c --licenses | Out-Null
            & $sm --sdk_root=$c 'platforms;android-36' 'build-tools;36.0.0' 'build-tools;34.0.0'
...
If Android Studio is installed, open it once and install through SDK Manager:
  ...
  - Android SDK Build-Tools 36.0.0 (build)
  - Android SDK Build-Tools 34.0.0 (reproducible APK signing)
...
$apksigner = Find-ReproApkSigner $SdkRoot
Write-Host "SDK: $SdkRoot"
Write-Host "Platform: $androidJar"
Write-Host "Build-tools: $bt"
Write-Host "Signing tool: $apksigner"
